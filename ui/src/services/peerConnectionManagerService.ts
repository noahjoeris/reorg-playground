...
export async function fetchPeerInfo(networkId: number, signal?: AbortSignal): Promise<PeerInfoResponse> {
  const res = await fetch(`/api/${networkId}/peer-info.json`, { signal })
  if (!res.ok) throw new Error(`fetchPeerInfo: ${res.status}`)
  return res.json()
}

export async function addNode(networkId: number, nodeId: number, address: string): Promise<RpcActionResponse> {
