...
export function useFaucet(network: Network) {
  ...
  const { trigger, isMutating } = useSWRMutation<FaucetResponse, Error, string, FaucetMutationArgs>(
    FAUCET_MUTATION_KEY,
    async (_key, { arg }) => {
  ...
  const send = useCallback(
    async (nodeId: number, nodeName: string, address: string, amountBtc: string) => {
      try {
