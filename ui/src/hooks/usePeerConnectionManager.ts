...
function peerConnectionsForNode(peerData: PeerInfoResponse, nodeId: number): PeerInfo[] {
 ...
 */
function peerMatchesPendingDisconnect(peer: PeerInfo, pendingDisconnect: PendingDisconnect): boolean {
  if (pendingDisconnect.matchedNodeId != null && peer.matched_node_id === pendingDisconnect.matchedNodeId) {
    return true
  }

  return peer.id === pendingDisconnect.peerId || peer.addr === pendingDisconnect.address
}
...
function resolvePendingConnects(
  ...
  const remaining = pendingConnects.filter(pendingConnect => {
    ...
    if (nodePeers.some(peer => peer.matched_node_id === pendingConnect.targetNodeId)) {
...
function resolvePendingDisconnects(
  ...
  const remaining = pendingDisconnects.filter(pendingDisconnect => {
    const nodePeers = peerConnectionsForNode(peerData, pendingDisconnect.nodeId)
    const stillConnected = nodePeers.some(peer => peerMatchesPendingDisconnect(peer, pendingDisconnect))
...
function buildPendingMutationIndexes(
  ...
  const pendingConnectionPairKeys = new Set(
    pendingConnects.map(pendingConnect => getNodePairKey(pendingConnect.nodeId, pendingConnect.targetNodeId)),
  )
  const disconnectingPeerKeys = new Set(
    pendingDisconnects.map(pendingDisconnect => `${pendingDisconnect.nodeId}:${pendingDisconnect.peerId}`),
  )
  const disconnectingConnectionPairKeys = new Set(
    pendingDisconnects.flatMap(pendingDisconnect =>
      pendingDisconnect.matchedNodeId == null
        ? []
        : [getNodePairKey(pendingDisconnect.nodeId, pendingDisconnect.matchedNodeId)],
    ),
  )
...
function hasPendingPeerMutations(pendingConnects: PendingConnect[], pendingDisconnects: PendingDisconnect[]): boolean {
  return pendingConnects.length > 0 || pendingDisconnects.length > 0
}

export function usePeerConnectionManager(network: Network | null, enabled = true) {
  ...
  const [pendingDisconnects, setPendingDisconnects] = useState<PendingDisconnect[]>([])
  ...
  useEffect(() => {
    if (!peerData || !hasPendingPeerMutations(pendingConnects, pendingDisconnects)) {
      return
    }

    const now = Date.now()
    const pendingConnectResolution = resolvePendingConnects(peerData, pendingConnects, now)
    const pendingDisconnectResolution = resolvePendingDisconnects(peerData, pendingDisconnects, now)

    if (pendingConnectResolution.completed.length > 0 || pendingConnectResolution.timedOut.length > 0) {
      setPendingConnects(pendingConnectResolution.remaining)
    }

    if (pendingDisconnectResolution.completed.length > 0 || pendingDisconnectResolution.timedOut.length > 0) {
  ...
  }, [peerData, pendingConnects, pendingDisconnects, notifyError, notifySuccess])
  ...
  useEffect(() => {
    ...
    eventSource.onerror = () => {
      setConnectionStatus(connectionStatusFromReadyState(eventSource.readyState))
    }
  ...
  const hasPendingMutations = hasPendingPeerMutations(pendingConnects, pendingDisconnects)
  ...
  const { pendingConnectionPairKeys, disconnectingPeerKeys, disconnectingConnectionPairKeys } = useMemo(
  ...
  const connectNodePair = useCallback(
    async (nodeId: number, nodeName: string, targetNodeId: number, targetLabel: string, address: string) => {
      if (!managedNetwork) {
        return false
      }

      const nodePairKey = getNodePairKey(nodeId, targetNodeId)
      if (pendingConnectionPairKeys.has(nodePairKey) || disconnectingConnectionPairKeys.has(nodePairKey)) {
        return false
      }
      ...
      try {
        ...
        setPendingConnects(current => current.filter(item => item.id !== pendingConnect.id))
        notifyError({
  ...
  const disconnectPeer = useCallback(
    async (
      ...
      const disconnectPairKey = matchedNodeId == null ? null : getNodePairKey(nodeId, matchedNodeId)
      if (
        disconnectingPeerKeys.has(disconnectKey) ||
        (disconnectPairKey != null && disconnectingConnectionPairKeys.has(disconnectPairKey))
      ) {
        return false
      }
      ...
      try {
        ...
        setPendingDisconnects(current => current.filter(item => item.id !== pendingDisconnect.id))
        notifyError({
