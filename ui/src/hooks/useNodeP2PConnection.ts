...
export function useNodeP2PConnection(network: Network | null, nodes: NodeInfo[] = []) {
  ...
  const [waitingForReconnectByNodeId, setWaitingForReconnectByNodeId] = useState<WaitingForReconnectByNodeId>({})
  const reconnectWaitTimeoutsRef = useRef<Record<number, ReturnType<typeof window.setTimeout>>>({})
