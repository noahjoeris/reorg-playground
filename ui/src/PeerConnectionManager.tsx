...
import { type PendingConnect, usePeerConnectionManager } from '@/hooks/usePeerConnectionManager'
import type { NodeP2PAddress, PeerInfo } from '@/services/peerConnectionManagerService'
...
function addnodeRemovalCandidates(peer: PeerInfo, nodeP2PAddresses: NodeP2PAddress[]): string[] {
  const catalog =
    peer.matched_node_id != null ? nodeP2PAddresses.find(a => a.node_id === peer.matched_node_id)?.address : undefined
    ...
    <Tooltip>
      <TooltipTrigger asChild>
        <span>
          ...
          >
            {showSpinner ? <Spinner className="size-3" /> : <ArrowDownUp className="size-3" />}
            {label}
          </Button>
        </span>
      </TooltipTrigger>
      <TooltipContent side="top" className="max-w-56 text-xs">
        Turn on/off the P2P networking. Reconnection can take ~30 seconds. Easy way to create reorgs.
      </TooltipContent>
    </Tooltip>
  )
}
...
function ConnectToNodeSelect({
  ...
  const handleSelectNode = async (targetNodeId: string) => {
    const target = availableTargets.find(t => String(t.node_id) === targetNodeId)
    if (!target) return
    setSelectedTargetNodeId(targetNodeId)
    setSubmitting(true)
    try {
      await onConnect(node.id, node.name, target.node_id, target.name, target.address)
    } finally {
  ...
  return (
    <Select value={selectedTargetNodeId} onValueChange={handleSelectNode} disabled={disabled || submitting}>
      <SelectTrigger size="sm" className="h-7 gap-1.5 px-2.5 text-xs" disabled={disabled || submitting}>
        {submitting ? <Spinner className="size-3" /> : <Plug className="size-3" />}
...
function isPeerDisconnecting(
  ...
  return peer.matched_node_id != null && disconnectingConnectionPairs.has(getNodePairKey(nodeId, peer.matched_node_id))
}
...
function NodePeerSection({
  ...
  const totalCount = peers.length + outgoingPendingConnectsForNode.length + incomingPendingConnectsForNode.length
  const connectionLabel = totalCount === 1 ? '1 connection' : `${totalCount} connections`
  const disconnectingPeerCount = peers.filter(peer =>
    isPeerDisconnecting(node.id, peer, disconnectingPeers, disconnectingConnectionPairs),
  ).length
  const hasAnyLoading =
    disconnectingPeerCount > 0 || outgoingPendingConnectsForNode.length > 0 || incomingPendingConnectsForNode.length > 0
  const canToggleP2P = !viewOnly && p2p.supported
  const peerActionsEnabled = canToggleP2P && p2p.active === true && !p2p.loading
  const waitingForReconnect = p2p.waitingForReconnect

  return (
    <Card
      className={['panel-glass gap-0 rounded-2xl py-0', !node.reachable && 'border-destructive/40 bg-destructive/10']
        .filter(Boolean)
        .join(' ')}
    >
      ...
      <CardContent className="space-y-2 px-3 pt-0 pb-2.5">
        {(peers.length > 0 ||
          ...
          <ul className="space-y-1">
                ...
                <PeerRow
                  ...
                  onDisconnect={() => {
                    ...
                    void onDisconnect(
                      ...
                      nodeP2PAddresses.filter(a => a.node_id === node.id).map(a => a.address),
                      peer.matched_node_id ?? null,
                    )
                  }}
                />
            ...
            {outgoingPendingConnectsForNode.map(pc => (
              <PendingConnectRow key={`pending-outgoing-${pc.id}`} label={`Connecting to ${pc.targetName}...`} />
            ))}
            {incomingPendingConnectsForNode.map(pc => (
              <PendingConnectRow key={`pending-incoming-${pc.id}`} label={`Connecting from ${pc.nodeName}...`} />
            ))}
            {waitingForReconnect && <PendingConnectRow label="Waiting for reconnection..." />}
          </ul>
        )}
        ...
        {peers.length === 0 &&
          outgoingPendingConnectsForNode.length === 0 &&
          incomingPendingConnectsForNode.length === 0 &&
          !waitingForReconnect && <p className="text-xs text-muted-foreground">{getEmptyStateLabel(p2p)}</p>}
        ...
        {canToggleP2P && availableTargets.length > 0 && (
          <>
            ...
            <ConnectToNodeSelect
              ...
              disabled={hasAnyLoading || waitingForReconnect || !peerActionsEnabled}
              onConnect={onAddNode}
            />
          </>
        )}
...
export function PeerConnectionManager({ network, nodes, getP2PControl, buttonClassName }: PeerConnectionManagerProps) {
  ...
  const peersByNodeId = new Map(peerData?.nodes.map(n => [n.node_id, n.peers]) ?? [])
  const nodeP2PAddresses = peerData?.node_p2p_addresses ?? []

  return (
    <>
      <Button variant="outline" size="xs" className={buttonClassName} onClick={() => setDialogOpen(true)}>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        ...
        >
          <DialogHeader>
            ...
            <DialogDescription>Manage connections between your nodes on {network.name}.</DialogDescription>
          ...
          {isLoading && !peerData ? (
            <div className="flex flex-col items-center justify-center gap-2 py-8">
              ...
              <p className="text-sm text-muted-foreground">Loading connection state...</p>
            </div>
          ) : (
  ...
  const availableTargets = useMemo(
    () =>
      nodeP2PAddresses.filter(
        a => a.node_id !== node.id && !connectedNodeIds.has(a.node_id) && !pendingRelatedNodeIds.has(a.node_id),
      ),
