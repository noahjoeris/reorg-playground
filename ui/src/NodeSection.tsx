...
import { ActiveNodeCard, activeTip, type P2PControl } from './ActiveNodeCard'
...
export function NodeSection({ network, nodes }: { network: Network; nodes: NodeInfo[] }) {
  ...
  const getP2PControl = useCallback(
    (node: NodeInfo): P2PControl => ({
      supported: p2pEnabledById[node.id] ?? false,
      active: getNodeP2PConnectionActive(node.id) ?? null,
      loading: p2pLoadingById[node.id] ?? false,
      waitingForReconnect: waitingForReconnectByNodeId[node.id] ?? false,
      onToggle: () => void toggleNodeP2PConnection(node),
    }),
    [p2pEnabledById, getNodeP2PConnectionActive, p2pLoadingById, toggleNodeP2PConnection, waitingForReconnectByNodeId],
  )
              ...
              {sortedNodes.length === 0 ? (
                ...
                <>
                  <div className="mb-2 flex items-center gap-2">
                    <PeerConnectionManager network={network} nodes={sortedNodes} getP2PControl={getP2PControl} />
                  </div>
                  <div className="flex max-h-184 flex-wrap gap-2 overflow-y-auto pb-1.5">
                    {sortedNodes.map(node => (
                      <ActiveNodeCard key={node.id} node={node} maxHeight={maxHeight} />
                    ))}
                  </div>
                </>
              )}
